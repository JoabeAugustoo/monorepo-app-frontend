...
export const stockService = {
  ...
  getLowStockProducts: async (minQuantity = 10) => {
    const response = await api.get(`/stocks/low-stock?minQuantity=${minQuantity}`);
    return response.data;
  },
  ...
  getMovementHistory: async (productPublicId: string): Promise<StockMovement[]> => {
    const response = await api.get(`/stock-movements/product/${productPublicId}`);
    return response.data;
  },
